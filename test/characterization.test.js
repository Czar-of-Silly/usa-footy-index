// Phase 5.3 characterization: the source split must reproduce the shipped grading outputs exactly.
// Fixture: test/fixtures/characterization.json, generated from the inline app BEFORE the split by
// scripts/make-characterization-fixture.js. Any difference here is a regression, not a "fix".
...
test("fixture and cache line up (skip-guard)", (t) => { if (!sameData) t.skip("cache moved on since the fixture was taken (" + cache.generated + " vs " + FX.generatedFrom + "); run npm run make-fixture after an intended change"); });

test("grades: every characterized player reproduces exactly", (t) => {
  if (!sameData) return t.skip();
...
test("grades: whole-pool distribution reproduces exactly", (t) => {
  if (!sameData) return t.skip();
...
test("computeForm reproduces exactly", (t) => {
  if (!sameData) return t.skip();
...
test("powerRankFor reproduces exactly", (t) => {
  if (!sameData) return t.skip();
