...
export function preparePlayerForGrading(r,i){const id="p"+i,m=r.m||600,p90=m/90;const games=Math.max(1,Math.round(m/90));return{id,tk90:(r.tk||0)/p90,tkwPct:(r.tk>=8?(r.tkw||0)/r.tk:0),blk90:(r.blk||0)/p90,xg90:(r.xg||0)/p90,xa90:(r.xa||0)/p90,pc:r.pp||75,pga:(r.gp||0)/p90,tga:((r.gs||0)+(r.gp||0)+(r.gdr||0)+(r.gdf||0)+(r.gi||0))/p90,dga:((r.gdf||0)+(r.gi||0))/p90,kp90:(r.kp||0)/p90,sca90:(r.sca||0)/p90,prgp90:(r.prgp||0)/p90,ftp90:(r.ftp||0)/p90,prs90:(r.prs||0)/p90,intc90:(r.intc||0)/p90,arl90:(r.arl||0)/p90,drb90:(r.drb||0)/p90,prgc90:(r.prgc||0)/p90,/*STEP4*/oxg90:(r.oxg||0)/p90,chc90:(r.chc||0)/p90,clr90:(r.clr||0)/p90,flSuf90:(r.flSuf||0)/p90,arlPctV:(r.arlPct||0),n90s:m/90,gdrV:(r.gdr||0)/p90,escV:(r.esc||0),presRV:(r.presR||0),passPerfV:(r.passPerf||0),
        // GK-specific
