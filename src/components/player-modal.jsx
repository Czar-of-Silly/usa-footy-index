...
export function PlayerModal({player:p,onClose,onCompare,isInCompare,allSeasons,currentSeason,viewingSeason,gkCoverage,onDrillSeason,canDrillSeason,pctRanks,history,formCurve,seasonInfo,similarPlayers,onSelectPlayer,dark:isDarkMode}){
  ...
  return <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.4)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16,backdropFilter:"blur(4px)"}}>
    <div onClick={e=>e.stopPropagation()} className="resp-modal" style={{background:T.bg,border:`1px solid ${T.border}`,borderRadius:0,maxWidth:860,maxHeight:"92vh",width:"calc(100% - 16px)",overflowY:"auto",animation:"modalIn .3s ease",boxShadow:"0 24px 60px rgba(0,0,0,.2)",position:"relative"}}>
      {/* Close */}
